...
s_wifi = dbus.Dictionary({
    'ssid': dbus.ByteArray("foobar".encode("utf-8")),
    'mode': "adhoc",
    'band': "bg",
...
ss_proxy = sys_bus.get_object('org.freedesktop.NetworkManager', '/org/freedesktop/NetworkManager/Settings')
ss_iface = dbus.Interface(ss_proxy, 'org.freedesktop.NetworkManager.Settings')
...
def find_connection(requested_uuid):
    for c in ss_iface.ListConnections():
        ...
        c_proxy = sys_bus.get_object('org.freedesktop.NetworkManager', c)
        c_iface = dbus.Interface(c_proxy, 'org.freedesktop.NetworkManager.Settings.Connection')
